            ...
            foreach ($periods as $column => $slot) {
                ...
                foreach (ticky_timetable_parse_grid_cell($raw, $where, $issues) as $part) {
                    $lessons[] = [
                        ...
                        // A cellában megadott csoportszám erosebb, mint a
                        // munkalapé: a nyelvi bontás alcsoportjai a cellában
                        // vannak felsorolva.
                        'csoport'     => $part['csoport'] ?? $title['csoport'],
                        'csoport_cellabol' => isset($part['csoport']),
                        'het_napja'   => $day,
                        'ora_sorszam' => $slot['ora_sorszam'],
                        'kezdes'      => $slot['kezdes'],
                        'vegzes'      => $slot['vegzes'],
 ...
 */
/**
 * Csoportonként külön sorba írt cella felismerése és feldolgozása.
 *
 *     csoportbontás
 *     any
 *     1. 208 MÉ
 ...
 *     4. 203 NYMK
 *
 * Minden számozott sor ÖNÁLLÓ óra, a sorszám pedig a csoport száma.
 *
 * Miért kell külön ág: az alapértelmezett rácscella-elemzo csak az UTOLSÓ
 * sort tekinti "terem tanár" párosnak. Ebben a formátumban ezért a "4."
 * sorszámot teremkódnak olvasta, a másik három csoport óráját pedig
 * elnyelte a tantárgynévbe. Mérve: egy 2279 cellás órarendbol 1545 óra
 * jött ki 2279 helyett, és 198 hamis tanárütközés keletkezett.
 *
 * @return array|null null, ha a cella NEM ilyen formátumú (marad a régi út)
 */
function ticky_timetable_parse_grid_cell_groups(
    array $lines,
    string $raw,
    string $where,
    array &$issues
): ?array {
    // A számozott sorok a cella VÉGÉN állnak, egymás után.
    $assignments = [];
    $index = count($lines) - 1;

    while ($index >= 0
        && preg_match('/^(\d+)\.\s+(\S.*)$/u', $lines[$index], $matches) === 1
        // A "1. csoport" fejléc nem hozzárendelés, hanem csoportjelölo.
        && preg_match('/^csoport\b/ui', $matches[2]) !== 1
    ) {
        array_unshift($assignments, ['csoport' => (int) $matches[1], 'szoveg' => $matches[2]]);
        $index--;
    }

    // Egyetlen számozott sor még lehet sima cella, ezért legalább ketto kell.
    if (count($assignments) < 2) {
        return null;
    }

    // A fejlécbol a "csoportbontás" jelölo nem tantárgynév.
    $fej = array_filter(
        array_slice($lines, 0, $index + 1),
        static fn(string $line): bool => preg_match('/^csoportbontás$/ui', $line) !== 1
    );

    $subject = ticky_timetable_clean_subject(array_values($fej));
    if ($subject === '') {
    ...
    foreach ($assignments as $assignment) {
        ...
        foreach (preg_split('#[\s/]+#u', $assignment['szoveg']) ?: [] as $token) {
            $token = trim($token, " \t.,;");
            if ($token === '') {
                continue;
            }
        ...
        if (count($rooms) !== 1 || count($teachers) !== 1) {
            $issues[] = ticky_timetable_issue(
                'error',
                'HIANYZO_TEREM_VAGY_TANAR',
                $where,
                'A(z) ' . $assignment['csoport'] . '. csoport sorából hiányzik a terem vagy a tanár: "'
                    . $assignment['szoveg'] . '"'
            );
            return [];
        }
...
function ticky_timetable_parse_grid_cell(string $raw, string $where, array &$issues): array
{
    ...
    // Csoportonként külön sorba írt cella – ha ilyen, ez kezeli le.
    $csoportos = ticky_timetable_parse_grid_cell_groups($lines, $raw, $where, $issues);
    if ($csoportos !== null) {
        return $csoportos;
    }
...
function ticky_timetable_dedupe_grid_rows(array $lessons, array $groups_by_class): array
{
    ...
    foreach ($buckets as $bucket) {
        ...
        // A cellában felsorolt alcsoport (nyelvi bontás) a sajátját tartja:
        // ott a csoportszám magából a cellából jön, nem a munkalap fejlécébol,
        // ezért nem szabad a munkalapok száma alapján elhagyni.
        if (empty($lesson['csoport_cellabol'])) {
