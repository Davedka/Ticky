...
<style>
  ...
  .filter-btn{transition:all .15s ease;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:rgba(255,255,255,.5);border-radius:10px;padding:8px 14px;font-size:13px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:6px;white-space:nowrap;}
  ...
  input[type=search]{background:rgba(255,255,255,.06);border:1.5px solid rgba(255,255,255,.10);color:white;border-radius:10px;padding:8px 14px 8px 34px;font-size:13px;width:100%;transition:border-color .2s;font-family:'DM Sans',sans-serif;}
  ...
  .nav-mobile-menu{display:none;position:fixed;top:64px;left:0;right:0;z-index:90;background:rgba(6,15,30,.97);backdrop-filter:blur(24px);border-bottom:1px solid rgba(255,255,255,.10);padding:12px 16px 16px;flex-direction:column;gap:4px;animation:slideDown .2s ease;}
  @keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:none}}
  .nav-mobile-menu.open{display:flex;}
  .nav-mobile-item{padding:12px 14px;border-radius:10px;font-size:14px;font-weight:500;color:rgba(255,255,255,.75);text-decoration:none;display:flex;align-items:center;gap:10px;}
  ...
  @media(max-width:768px){
    ...
    .filter-area{flex-direction:column;gap:10px;}
    .search-row{width:100%!important;}
  }
  @media(max-width:480px){
    .rooms-grid-outer{grid-template-columns:repeat(2,1fr)!important;}
  }
</style>
</head>
<body>
...
<div class="mobile-bottom-bar">
  ...
  <a href="/termek" class="mbb-item active"><span class="mbb-icon">🏫</span>Termek</a>
...
<div class="nav-mobile-menu" id="nav-mobile">
  <a href="/" class="nav-mobile-item">🏠 Főoldal</a>
  ...
  <a href="/admin" class="nav-mobile-item" style="color:#f0c76b;">⚙️ Admin</a>
</div>

<nav style="background:rgba(6,15,30,.7);backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,.07);" class="sticky top-0 z-50 px-4 sm:px-5 h-16 flex items-center justify-between">
  <div class="flex items-center gap-2 sm:gap-3 min-w-0">
    <a href="/" style="font-family:'Playfair Display',serif;color:white;font-size:18px;font-weight:700;" class="flex items-center gap-2 flex-shrink-0">
    ...
    <span style="color:rgba(255,255,255,.2);" class="nav-links">·</span>
    <span class="text-sm nav-links" style="color:rgba(255,255,255,.45);">Összes terem</span>
  </div>
  <div class="flex items-center gap-2">
    <button onclick="refresh()" class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs" style="color:rgba(255,255,255,.4);border:1px solid rgba(255,255,255,.12);background:transparent;font-size:12px;">
      ...
      <span id="footer-ido" class="hidden sm:inline">–</span>
    </button>
...
<div class="relative z-10 max-w-5xl mx-auto px-3 sm:px-4 pt-4 sm:pt-5 pb-3">
  <div class="filter-area flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
    <div class="flex items-center gap-2 flex-wrap">
      <button class="filter-btn active" id="btn-mind" onclick="setFilter('mind')">Összes <span id="cnt-mind" style="font-family:'Playfair Display',serif;font-weight:700;font-size:14px;">–</span></button>
      <button class="filter-btn" id="btn-szabad" onclick="setFilter('szabad')"><span class="w-2 h-2 rounded-full flex-shrink-0" style="background:#4ade80;display:inline-block;"></span>Szabad <span id="cnt-szabad" style="font-family:'Playfair Display',serif;font-weight:700;font-size:14px;">–</span></button>
      <button class="filter-btn" id="btn-foglalt" onclick="setFilter('foglalt')"><span class="w-2 h-2 rounded-full flex-shrink-0" style="background:#ff6b82;display:inline-block;"></span>Foglalt <span id="cnt-foglalt" style="font-family:'Playfair Display',serif;font-weight:700;font-size:14px;">–</span></button>
    </div>
    <div class="relative search-row w-full sm:w-48">
...
<main class="relative z-10 max-w-5xl mx-auto px-3 sm:px-4 pb-16">
  <div id="skeleton-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
    <div class="skeleton h-28 sm:h-32 rounded-2xl"></div><div class="skeleton h-28 sm:h-32 rounded-2xl"></div>
    <div class="skeleton h-28 sm:h-32 rounded-2xl"></div><div class="skeleton h-28 sm:h-32 rounded-2xl"></div>
    <div class="skeleton h-28 sm:h-32 rounded-2xl"></div><div class="skeleton h-28 sm:h-32 rounded-2xl"></div>
    <div class="skeleton h-28 sm:h-32 rounded-2xl"></div><div class="skeleton h-28 sm:h-32 rounded-2xl"></div>
  </div>
  <div id="rooms-grid" class="hidden rooms-grid-outer grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3"></div>
...
<div id="modal-overlay" class="hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center p-3 sm:p-4" style="background:rgba(6,15,30,.7);backdrop-filter:blur(8px);" onclick="handleOverlayClick(event)">
...
document.addEventListener('click',e=>{const m=document.getElementById('nav-mobile');if(m&&!m.contains(e.target)&&!e.target.closest('.nav-burger')&&m.classList.contains('open'))m.classList.remove('open')})
async function fetchRooms(){
  try{
    ...
    if(d.nap===0){const wi=document.getElementById('weekend-info');if(wi)wi.style.display='flex'}
...
function updateCounts(){
  const sz=allRooms.filter(r=>r.allapot==='szabad').length,fo=allRooms.filter(r=>r.allapot==='foglalt').length
...
function setFilter(f){
  curFilter=f
  ;['mind','szabad','foglalt'].forEach(k=>{const b=document.getElementById('btn-'+k);b.className='filter-btn'+(k===f?(f==='szabad'?' active-szabad':f==='foglalt'?' active-foglalt':' active'):'')})
  renderGrid()
}
function filterRooms(){curSearch=(document.getElementById('search-input').value||'').toLowerCase();renderGrid()}
...
function renderGrid(){
  ...
  grid.innerHTML=rooms.map((r,i)=>{
    ...
    let body=''
    if(sz){body=`<p class="text-xs mt-2" style="color:rgba(255,255,255,.3);">Nincs óra</p>`}
    else{const a=r.aktualis,pct=calcPct(a.kezdes,a.vegzes);body=`<p class="text-xs font-medium mt-2 truncate" style="color:rgba(255,255,255,.7);">${a.tanar} · ${a.osztaly}</p><p class="text-xs truncate" style="color:rgba(255,255,255,.35);">${a.tantargy} · ${a.kezdes}–${a.vegzes}</p><div class="mt-2 h-1 rounded-full overflow-hidden" style="background:rgba(255,255,255,.1);"><div class="h-full rounded-full" style="width:${pct}%;background:linear-gradient(90deg,#e8334a,#ff6b82);"></div></div>`}
    const pillBg=sz?'rgba(26,138,74,.2)':'rgba(200,16,46,.2)',pillColor=sz?'#4ade80':'#ff6b82',dotBg=sz?'#4ade80':'#ff6b82',pillTxt=sz?'SZABAD':'FOGLALT'
    return `<div class="room-card glass card-in rounded-2xl p-3 sm:p-4" style="animation-delay:${i*25}ms;" onclick="openModal('${r.terem_szam}')">
      <div class="flex items-start justify-between gap-1">
        <div><p class="text-xs font-semibold tracking-widest uppercase" style="color:rgba(255,255,255,.3);">Terem</p>
        <h2 style="font-family:'Playfair Display',serif;font-size:24px;font-weight:700;color:white;line-height:1.1;">${r.terem_szam}</h2></div>
        <div class="flex items-center gap-1 px-2 py-1 rounded-full flex-shrink-0" style="background:${pillBg};border:1px solid ${pillColor}33;font-size:9px;font-weight:700;color:${pillColor};letter-spacing:.05em;">
          <span class="w-1.5 h-1.5 rounded-full pulse flex-shrink-0" style="background:${dotBg};display:inline-block;"></span>${pillTxt}
        </div>
      </div>${body}</div>`
  }).join('')
}
