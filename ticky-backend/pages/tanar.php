...
<div class="w-full max-w-sm slide-up">
  <div class="glass rounded-2xl overflow-hidden">
    ...
    <div class="px-5 sm:px-6 pt-4 pb-5 card-inner" style="border-bottom:1px solid rgba(255,255,255,.08);">
      ...
      <select id="sel" class="custom-select" onchange="onSelect()"><option value="">— Válassz tanárt —</option></select>
    </div>
    <!-- Aktuális -->
    <div class="px-5 sm:px-6 py-5" id="aktblock" style="border-bottom:1px solid rgba(255,255,255,.08);">
      <div class="text-center py-3"><span style="font-size:36px;" class="block mb-2">👆</span><p class="text-sm" style="color:rgba(255,255,255,.4);">Válassz tanárt a legördülő menüből</p></div>
    </div>
    <!-- Napirend -->
    ...
    <div class="px-5 sm:px-6 py-4 flex items-center justify-between gap-2" style="border-top:1px solid rgba(255,255,255,.08);">
      ...
      <button onclick="refresh()" class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg" style="color:rgba(255,255,255,.4);border:1px solid rgba(255,255,255,.10);background:transparent;font-size:12px;">
...
<script>
const REFRESH=60_000; let curKod=null
...
function getUrlKod(){const p=location.pathname.split('/').filter(Boolean);const q=new URLSearchParams(location.search).get('tanar');if(p[0]==='tanar'&&p[1])return decodeURIComponent(p[1]).toUpperCase();if(q)return decodeURIComponent(q).toUpperCase();return null}
async function loadTanarok(){
  try{const d=await fetch('/api/tanarok').then(r=>r.json());const sel=document.getElementById('sel');(d.tanarok||[]).forEach(t=>{const o=document.createElement('option');o.value=t.rovid_nev;o.textContent=t.nev?`${t.rovid_nev} – ${t.nev}`:t.rovid_nev;sel.appendChild(o)});const url=getUrlKod();if(url){sel.value=url;if(sel.value){curKod=url;loadData()}}}catch(e){}
}
function onSelect(){const v=document.getElementById('sel').value;if(!v){curKod=null;reset();return};curKod=v;history.replaceState(null,'','/tanar/'+encodeURIComponent(v));loadData()}
function reset(){document.getElementById('aktblock').innerHTML=`<div class="text-center py-3"><span style="font-size:36px;" class="block mb-2">👆</span><p class="text-sm" style="color:rgba(255,255,255,.4);">Válassz tanárt a legördülő menüből</p></div>`;document.getElementById('lista').innerHTML=`<p class="text-sm" style="color:rgba(255,255,255,.3);">Nincs kiválasztva tanár</p>`;setAllapot('idle')}
function setAllapot(a){
  ...
  if(a==='tant'){document.body.className='tant';pill.style.cssText='display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:9999px;font-size:11px;font-weight:600;background:rgba(200,16,46,.25);color:#ff6b82;border:1px solid rgba(200,16,46,.4);flex-shrink:0;';dot.style.background='#ff6b82';txt.textContent='TANÍT'}
  else if(a==='szabad'){document.body.className='szabad';pill.style.cssText='display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:9999px;font-size:11px;font-weight:600;background:rgba(26,138,74,.25);color:#4ade80;border:1px solid rgba(26,138,74,.4);flex-shrink:0;';dot.style.background='#4ade80';txt.textContent='SZABAD'}
  else{document.body.className='';pill.style.cssText='display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:9999px;font-size:11px;font-weight:600;background:rgba(255,255,255,.08);color:rgba(255,255,255,.4);flex-shrink:0;';dot.style.background='rgba(255,255,255,.3)';txt.textContent='–'}
}
...
async function loadData(){
  ...
  try{
    ...
    if(d.error){document.getElementById('aktblock').innerHTML=`<div class="text-center py-3"><span style="font-size:32px;" class="block mb-2">⚠️</span><p class="text-sm" style="color:rgba(255,255,255,.4);">${d.error}</p></div>`;setAllapot('idle');return}
    const orak=d.orak||[];const akt=orak.find(o=>isAktiv(o.kezdes,o.vegzes));const kov=orak.find(o=>!isMult(o.vegzes)&&!isAktiv(o.kezdes,o.vegzes))
    setAllapot(akt?'tant':orak.length>0?'szabad':'idle');renderAkt(akt,kov);renderLista(orak)
  ...
  }catch(e){document.getElementById('aktblock').innerHTML=`<div class="text-center py-3"><span style="font-size:32px;" class="block mb-2">⚠️</span><p class="text-sm" style="color:rgba(255,255,255,.4);">Betöltési hiba</p></div>`;setAllapot('idle')}
  updateTime()
}
function renderAkt(a,k){
  const el=document.getElementById('aktblock')
  if(a){const p=calcPct(a.kezdes,a.vegzes);el.innerHTML=`<div class="flex flex-col gap-4 aktiv-card"><div><p class="text-xs font-semibold tracking-widest uppercase mb-0.5" style="color:rgba(255,255,255,.3);">Most itt van</p><p style="font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:white;line-height:1.1;">${a.terem}. terem</p></div><div class="grid grid-cols-2 gap-3"><div><p class="text-xs font-semibold tracking-widest uppercase mb-0.5" style="color:rgba(255,255,255,.3);">Osztály</p><p style="font-family:'Playfair Display',serif;font-size:17px;font-weight:700;color:white;">${a.osztaly}</p></div><div><p class="text-xs font-semibold tracking-widest uppercase mb-0.5" style="color:rgba(255,255,255,.3);">Tantárgy</p><p style="font-family:'Playfair Display',serif;font-size:17px;font-weight:700;color:white;">${a.tantargy}</p></div></div><div><div class="h-1.5 rounded-full overflow-hidden" style="background:rgba(255,255,255,.08);"><div class="h-full rounded-full" style="width:${p}%;background:linear-gradient(90deg,#e8334a,#ff6b82);transition:width .6s ease;"></div></div><div class="flex justify-between mt-1.5 text-xs" style="color:rgba(255,255,255,.35);"><span>${a.kezdes}</span><span style="color:#ff6b82;font-weight:600;">${a.vegzes}-ig</span><span>${a.vegzes}</span></div></div></div>`}
  else if(k){el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card"><span style="font-size:28px;">☕</span><div><p class="font-semibold" style="color:rgba(255,255,255,.8);">Jelenleg szabad</p><p class="text-sm mt-0.5" style="color:rgba(255,255,255,.4);">Következő: <strong style="color:rgba(255,255,255,.7);">${k.terem}. terem</strong> · ${k.kezdes}–${k.vegzes}</p></div></div>`}
  else{el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card"><span style="font-size:28px;">✅</span><div><p class="font-semibold" style="color:#4ade80;">Szabad</p><p class="text-sm mt-0.5" style="color:rgba(255,255,255,.4);">Ma már nincs több óra</p></div></div>`}
}
function renderLista(orak){
  ...
  el.innerHTML=`<div class="lista-in">`+orak.map((o,i)=>{
    ...
    return `<div class="ora-row${ak?' aktiv':mu?' mult':''} flex items-center gap-3 px-2 py-2.5 -mx-1">
      ...
      <div class="flex-1 min-w-0"><div class="flex items-baseline gap-1.5 flex-wrap"><span class="text-sm font-medium" style="color:${mu?'rgba(255,255,255,.3)':'rgba(255,255,255,.8)'};">${o.terem}. terem</span><span class="text-xs" style="color:rgba(255,255,255,.35);">${o.osztaly} · ${o.tantargy}</span></div><p class="text-xs" style="color:rgba(255,255,255,.28);">${o.kezdes} – ${o.vegzes}</p></div>
...
loadTanarok();setInterval(()=>{if(curKod)loadData()},REFRESH)
</script>
</body>
</html>
