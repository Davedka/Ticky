...
<div class="w-full max-w-sm slide-up relative z-10 mt-6">
  <div class="glass rounded-2xl overflow-hidden">
    ...
    <div class="px-6 pt-4 pb-0 flex items-center gap-2">
      <a href="/" style="font-family:'Playfair Display',serif;color:rgba(255,255,255,.35);font-size:14px;font-weight:700;text-decoration:none;display:flex;align-items:center;gap:6px;">
    ...
    <!-- Fejléc -->
    <div class="px-6 pt-4 pb-5" style="border-bottom:1px solid rgba(255,255,255,.08);">
...
async function loadData() {
  ...
  try {
    ...
    if(d.szunet){
      document.getElementById('aktblock').innerHTML=`<div style="text-align:center;padding:32px 16px;"><div style="font-size:48px;margin-bottom:12px;">🌙</div><h3 style="font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:#f0c76b;margin-bottom:8px;">${d.szunet}</h3><p style="font-size:14px;color:rgba(255,255,255,.5);">Szünet idején nincs tanítás</p></div>`
      document.getElementById('napiblock').innerHTML=''
      setAllapot('idle'); return
    }
    if(d.uzenet&&!d.orak?.length){
      document.getElementById('aktblock').innerHTML=`<div style="text-align:center;padding:32px 16px;"><div style="font-size:36px;margin-bottom:8px;">🌙</div><p style="font-size:14px;color:rgba(255,255,255,.45);">${d.uzenet}</p></div>`
      document.getElementById('napiblock').innerHTML=''
      setAllapot('idle'); return
    }
    if(d.error){
  ...
  document.getElementById('ido').textContent=new Date().toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit',second:'2-digit'})
}
...
function renderAkt(a,k) {
  ...
  if(a){
    ...
    el.innerHTML=`<div class="flex flex-col gap-4 aktiv-card">
      <div>
        <p class="text-xs font-semibold tracking-widest uppercase mb-0.5" style="color:rgba(255,255,255,.3);">Most itt van</p>
        <p style="font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:white;line-height:1.1;">${a.terem}. terem</p>
      </div>
    ...
    el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card">
      <span style="font-size:28px;">☕</span>
      <div>
        <p class="font-semibold" style="color:rgba(255,255,255,.8);">Jelenleg szabad</p>
        <p class="text-sm mt-0.5" style="color:rgba(255,255,255,.4);">Következő: <strong style="color:rgba(255,255,255,.7);">${k.terem}. terem</strong> · ${k.kezdes}–${k.vegzes}</p>
      </div>
    </div>`
  } else {
    el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card">
...
function renderLista(orak) {
  ...
  el.innerHTML=`<div class="lista-in">`+orak.map((o,i)=>{
    ...
    return `<div class="ora-row${ak?' aktiv':mu?' mult':''} flex items-center gap-3 px-3 py-2.5 -mx-1">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-baseline gap-1.5 flex-wrap">
          ...
          <span class="text-xs" style="color:rgba(255,255,255,.35);">${o.osztaly} · ${o.tantargy}</span>
        </div>
...
// ── Init ─────────────────────────────────────────────
// Live clock
function tickIdo(){const el=document.getElementById('ido');if(el)el.textContent=new Date().toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit',second:'2-digit'});}
setInterval(tickIdo,1000);tickIdo();
