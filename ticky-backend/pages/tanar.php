...
<style>
  ...
  .custom-select{width:100%;padding:12px 40px 12px 16px;border-radius:10px;
...
function renderAkt(a,k){
  ...
  if(a){
    ...
    el.innerHTML=`<div class="flex flex-col gap-4 aktiv-card">
      <div><p class="text-xs font-semibold tracking-widest uppercase mb-0.5" style="color:rgba(255,255,255,.3);">Most itt van</p><p style="font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:white;line-height:1.1;">${a.terem}. terem</p></div>
    ...
    el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card"><span style="font-size:28px;">☕</span><div><p class="font-semibold" style="color:rgba(255,255,255,.8);">Jelenleg szabad</p><p class="text-sm mt-0.5" style="color:rgba(255,255,255,.4);">Következő: <strong style="color:rgba(255,255,255,.7);">${k.terem}. terem</strong> · ${k.kezdes}–${k.vegzes}</p></div></div>`
  }else{
    el.innerHTML=`<div class="flex items-center gap-4 py-1 aktiv-card"><span style="font-size:28px;">✅</span><div><p class="font-semibold" style="color:#4ade80;">Szabad</p><p class="text-sm mt-0.5" style="color:rgba(255,255,255,.4);">Ma már nincs több óra</p></div></div>`
  }
}
