...
<style>
  ...
  .ora-blk{position:absolute;left:3px;right:3px;border-radius:8px;padding:5px 7px;overflow:hidden;border:1px solid rgba(255,255,255,.08);transition:filter .15s,transform .15s;cursor:default;background:linear-gradient(160deg,rgba(26,74,138,.85),rgba(11,46,89,.9));}
...
const START=7*60+30,END=14*60+30,TOTAL=END-START,PPM=2
...
function buildTT(){
  ...
  for(let n=1;n<=5;n++){const isMa=n===mai;html+=`<div class="tt-hdr${isMa?' ma':''}">${NAP[n]}${isMa?' <span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#c8972a;margin-left:4px;vertical-align:middle;animation:pd 2s infinite;"></span>':''}</div>`}
  ...
  for(let n=1;n<=5;n++){
    ...
    orak.forEach(o=>{
      const top=topPx(toMin(o.kezdes)),h=Math.max(24,(toMin(o.vegzes)-toMin(o.kezdes))*PPM)
      const ak=isMa&&isAktiv(o.kezdes,o.vegzes),mu=isMa&&isMult(o.vegzes),p=ak?pct(o.kezdes,o.vegzes):0
      const nm=o.tanar_nev||o.tanar
      col+=`<div class="ora-blk ${ak?'aktiv':mu?'mult':''}" style="top:${top}px;height:${h}px;" title="${nm} · ${o.osztaly} · ${o.tantargy} · ${o.kezdes}–${o.vegzes}"><div class="ob-tanar">${nm}</div>${h>38?`<div class="ob-meta">${o.osztaly} · ${o.tantargy}</div>`:''} ${h>56?`<div class="ob-meta">${o.kezdes}–${o.vegzes}</div>`:''} ${ak?`<div class="ob-prog"><div class="ob-prog-fill" style="width:${p}%;"></div></div>`:''}</div>`
    })
    col+=`</div>`;html+=col
  }
...
function build(){buildSummary();buildTT();buildMobTabs();buildMobList();if(nowTimer)clearInterval(nowTimer);nowTimer=setInterval(()=>{const nl=document.getElementById('now-line');if(nl){const nm=nowM();if(nm>=START&&nm<=END)nl.style.top=topPx(nm)+'px'};buildSummary()},60_000)}
function showErr(msg){document.getElementById('tt-skel').style.display='none';document.getElementById('tt-outer').innerHTML=`<div style="text-align:center;padding:60px 20px;position:relative;z-index:10;"><span style="font-size:40px;">⚠️</span><p style="color:rgba(255,255,255,.5);margin-top:12px;">${msg}</p></div>`}
