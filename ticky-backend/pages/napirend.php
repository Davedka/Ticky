...
<!-- Ticky brand – felül, komponensen belül -->
<div class="page-header" style="padding-top:16px;padding-bottom:4px;">
  <a href="/" style="font-family:'Playfair Display',serif;color:rgba(255,255,255,.3);font-size:13px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:6px;margin-bottom:12px;">
    <span style="width:5px;height:5px;border-radius:50%;background:#c8972a;display:inline-block;"></span>
    Ticky
  </a>
</div>
...
async function fetchData() {
  try {
    ...
    if(d.szunet){
      ...
      document.getElementById('tt-outer').innerHTML=`<div style="text-align:center;padding:60px 20px;position:relative;z-index:10;"><div style="font-size:56px;margin-bottom:16px;">🌙</div><p style="font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:#f0c76b;margin-bottom:8px;">${d.szunet}</p><p style="color:rgba(255,255,255,.4);">Szünet idején nincs tanítás</p></div>`
      return
    }
    hetData={}
...
if(!teremSzam){
  ...
  fetchData()
  setInterval(()=>{const el=document.getElementById('footer-ido');if(el)el.textContent=new Date().toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit',second:'2-digit'});},1000)
