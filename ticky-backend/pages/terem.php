...
<style>
  :root{
    ...
    --txt:#fff; --dim:rgba(255,255,255,.55); --faint:rgba(255,255,255,.3);
    --border:rgba(255,255,255,.10); --border-strong:rgba(255,255,255,.18);
    --glass:rgba(255,255,255,.05);
  }
  *{box-sizing:border-box;}
  body{
    font-family:'DM Sans',sans-serif; color:var(--txt); min-height:100vh;
    background-color:#060f1e; transition:background-image .6s ease; margin:0;
  ...
  .glass { background:var(--glass);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid var(--border);position:relative; }
  .glass::before{content:'';position:absolute;inset:0 0 auto 0;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.14),transparent);pointer-events:none;}
  ...
  .pill{display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:9999px;font-size:14px;font-weight:600;flex-shrink:0;background:rgba(255,255,255,.08);color:var(--dim);transition:background .3s,color .3s;}
  .pill .dot{width:8px;height:8px;border-radius:50%;background:rgba(255,255,255,.3);flex-shrink:0;}
  .pill.is-foglalt{background:rgba(200,16,46,.25);color:var(--red);border:1px solid rgba(200,16,46,.4);} .pill.is-foglalt .dot{background:var(--red);}
  .pill.is-szabad{background:rgba(26,138,74,.25);color:var(--green);border:1px solid rgba(26,138,74,.4);} .pill.is-szabad .dot{background:var(--green);}

  .icon-btn{display:inline-flex;align-items:center;gap:6px;padding:8px 13px;border-radius:9px;font-size:12px;font-family:'DM Sans',sans-serif;color:var(--dim);border:1px solid var(--border);background:rgba(255,255,255,.03);cursor:pointer;transition:background .15s,color .15s,border-color .15s;}
  .icon-btn:hover{background:rgba(255,255,255,.08);color:#fff;border-color:var(--border-strong);}
  .icon-btn svg{width:13px;height:13px;}
  .link-gold{color:var(--gold-l);font-weight:500;display:inline-flex;align-items:center;gap:5px;font-size:12px;}
  ...
  .tt-hdr { padding:7px 4px 9px;text-align:center;font-size:10px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;color:var(--faint);border-bottom:1px solid rgba(255,255,255,.07); }
  ...
  .tlabel { position:absolute;right:4px;font-family:'DM Mono',monospace;font-size:9px;font-weight:500;color:rgba(255,255,255,.22);transform:translateY(-50%);white-space:nowrap; }
  .ora-blk {
    position:absolute;left:2px;right:2px;border-radius:7px;padding:4px 6px;overflow:hidden;
    border:1px solid rgba(255,255,255,.07);transition:filter .15s,transform .15s;cursor:pointer;
    background:linear-gradient(160deg,rgba(26,74,138,.8),rgba(11,46,89,.85));
  }
  .ora-blk:hover { filter:brightness(1.2);transform:scale(1.01);z-index:20; }
  ...
  .ob-tanar { font-family:'Playfair Display',serif;font-size:11px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.2; }
  ...
  .now-line { position:absolute;left:0;right:0;height:2px;pointer-events:none;z-index:15;background:linear-gradient(90deg,transparent,var(--red) 20%,var(--red) 80%,transparent); }
  .now-dot { position:absolute;left:0;top:-4px;width:8px;height:8px;border-radius:50%;background:var(--red);box-shadow:0 0 6px var(--red);animation:pd 1.5s infinite; }
  ...
  .next-card{margin-top:16px;border-radius:14px;padding:14px 16px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);}
  .lbl-up{font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:var(--faint);}
</style>
</head>
<body>
...
<div class="relative z-10 max-w-sm mx-auto px-4 pt-8 slide-up">
  <div class="glass rounded-2xl overflow-hidden">
    ...
    <div class="px-7 pt-4 pb-0 flex items-center justify-between">
      <a href="/" style="font-family:'Playfair Display',serif;color:rgba(255,255,255,.4);font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px;">
        <span style="width:5px;height:5px;border-radius:50%;background:var(--gold);display:inline-block;"></span>
        Ticky
      </a>
      <a href="/termek" class="link-gold" style="color:var(--dim);">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
        Összes terem
      </a>
    </div>

    <div class="px-7 pt-6 pb-5 flex items-center justify-between gap-3" style="border-bottom:1px solid var(--border);">
      <div>
        <p class="lbl-up mb-1">Terem</p>
        <h1 id="terem-szam" style="font-family:'Playfair Display',serif;font-size:52px;font-weight:700;color:#fff;line-height:1;letter-spacing:-1px;">–</h1>
      </div>
    ...
    <div class="px-7 py-4 flex items-center justify-between gap-2" style="border-top:1px solid var(--border);">
      <span style="font-family:'DM Mono',monospace;font-size:12px;color:rgba(255,255,255,.4);" id="footer-ido">–</span>
      <button onclick="refresh()" class="icon-btn">
...
<div class="tt-title">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
    <p class="lbl-up">Heti órarend</p>
    <a id="napirend-link" href="#" class="link-gold">
      Teljes nézet
...
const IC_CHECK = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:30px;height:30px;margin:0 auto 10px;display:block;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>'
const IC_MOON  = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:34px;height:34px;margin:0 auto 10px;display:block;"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>'
const IC_SEARCH= '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:30px;height:30px;margin:0 auto 12px;display:block;"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>'
...
function setAllapot(a) {
  ...
  if(a==='foglalt'){
    ...
    pill.className='pill is-foglalt'; txt.textContent='FOGLALT'
  } else {
    document.body.className='szabad'
    pill.className='pill is-szabad'; txt.textContent='SZABAD'
  }
}

function kovHtml(k) {
  if(!k) return `<div class="next-card"><p class="lbl-up" style="margin-bottom:4px;">Következő óra</p><p style="font-size:14px;color:rgba(255,255,255,.35);">Ma már nincs több óra</p></div>`
  return `<div class="next-card"><p class="lbl-up" style="margin-bottom:6px;">Következő óra</p><div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;"><span style="font-size:14px;font-weight:500;color:rgba(255,255,255,.7);">${k.tanar} · ${k.osztaly} · ${k.tantargy}</span><span style="font-family:'DM Mono',monospace;font-size:12px;color:rgba(255,255,255,.35);">${k.kezdes}–${k.vegzes}</span></div></div>`
}
...
function renderStatus(data) {
  ...
  if(data.allapot==='szabad'){
    el.innerHTML=`<div style="text-align:center;padding:12px 0;color:var(--green);">${IC_CHECK}<p style="font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:var(--green);" class="mb-1">Szabad terem</p><p style="font-size:14px;color:rgba(255,255,255,.4);">Nincs aktív foglalás</p></div>${kovHtml(data.kovetkezo)}`
  } else {
    const a=data.aktualis
    const pct=calcPct(a.kezdes,a.vegzes)
    el.innerHTML=`<div class="flex flex-col gap-4">
      <div><p class="lbl-up" style="margin-bottom:2px;">Tanár</p><p style="font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:#fff;line-height:1.2;">${a.tanar_nev||a.tanar}</p></div>
      <div class="grid grid-cols-2 gap-3">
        <div><p class="lbl-up" style="margin-bottom:2px;">Osztály</p><p style="font-family:'Playfair Display',serif;font-size:18px;font-weight:700;color:#fff;">${a.osztaly}</p></div>
        <div><p class="lbl-up" style="margin-bottom:2px;">Tantárgy</p><p style="font-family:'Playfair Display',serif;font-size:18px;font-weight:700;color:#fff;">${a.tantargy}</p></div>
      </div>
      <div>
        <div class="h-1.5 rounded-full overflow-hidden" style="background:rgba(255,255,255,.08);">
          <div class="h-full rounded-full prog-bar" style="width:${pct}%;background:linear-gradient(90deg,var(--red-deep),var(--red));"></div>
        </div>
        <div class="flex justify-between mt-1.5" style="font-family:'DM Mono',monospace;font-size:12px;color:rgba(255,255,255,.35);">
          <span>${a.kezdes}</span><span style="color:var(--red);font-weight:600;">még ${a.perc_maradt} perc</span><span>${a.vegzes}</span>
        </div>
      </div>
...
function buildTT() {
  ...
  for(let n=1;n<=5;n++){
    html+=`<div class="tt-hdr${n===mai?' ma':''}">${NAP[n]}${n===mai?'<span style="display:inline-block;width:4px;height:4px;border-radius:50%;background:var(--gold);margin-left:3px;vertical-align:middle;animation:pd 2s infinite;"></span>':''}</div>`
  }
  ...
  for(let n=1;n<=5;n++){
    ...
    orak.forEach(o=>{
      ...
      col+=`<a href="${href}" class="ora-blk${ak?' aktiv':mu?' mult':''}" style="top:${top}px;height:${h}px;"
...
async function fetchStatus() {
  try {
    ...
    if(!d.error){
      ...
      if(d.szunet){
        setAllapot('szabad')
        document.getElementById('content').innerHTML=`<div style="text-align:center;padding:24px 0;color:var(--gold-l);">${IC_MOON}<p style="font-family:'Playfair Display',serif;font-size:18px;font-weight:700;color:var(--gold-l);margin-bottom:4px;">${d.szunet}</p><p style="font-size:13px;color:rgba(255,255,255,.4);">Szünet idején nincs tanítás</p></div>`
      } else {
        renderStatus(d)
      }
    }
...
if(!teremSzam){
  ...
  document.getElementById('content').innerHTML=`<div style="text-align:center;padding:24px 0;color:rgba(255,255,255,.6);">${IC_SEARCH}<p style="color:rgba(255,255,255,.6);">Nincs terem megadva</p><p style="font-family:'DM Mono',monospace;font-size:13px;margin-top:4px;color:rgba(255,255,255,.35);">URL: /terem/204</p></div>`
} else {
