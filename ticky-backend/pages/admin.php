...
function ticky_get_visits(): array {
    ...
    $week_cnt  = array_sum(array_filter($data['days'] ?? [], fn($d) => $d >= $week_start, ARRAY_FILTER_USE_KEY));
    return ['today' => $today_cnt, 'week' => $week_cnt, 'total' => $data['total'] ?? 0, 'days' => $data['days'] ?? []];
}

$visits = ticky_get_visits();

// ── Szünet ────────────────────────────────────────────────────────────
$aktiv_szunet    = aktiv_szunet();
$kovetkezo_szunet = null;
$ma = date('Y-m-d');
foreach (TICKY_SZUNETEK as $sz) {
    if ($sz['start'] > $ma) { $kovetkezo_szunet = $sz; break; }
}
?>
<!DOCTYPE html>
<html lang="hu">
...
<style>
  ...
  .inp{width:100%;padding:10px 14px;border-radius:8px;border:1.5px solid rgba(255,255,255,.10);background:rgba(255,255,255,.05);color:white;font-family:'DM Sans',sans-serif;font-size:14px;transition:border-color .2s,box-shadow .2s;}
  ...
  /* FIX: edit form highlight – a névmező villog */
  @keyframes inputPulse{0%,100%{border-color:rgba(255,255,255,.10);box-shadow:none}40%{border-color:#f0c76b;box-shadow:0 0 0 3px rgba(200,151,42,.2)}}
  .input-highlight{animation:inputPulse .7s ease 2;}
  @media(max-width:768px){.sidebar{display:none;}.content{padding:16px;height:auto;}}
</style>
</head>
<body>
...
<div class="layout">
  ...
  <main class="content" id="main-content">
    ...
    <section class="section active" id="section-dashboard">
      ...
      <!-- ── LÁTOGATÓI STATISZTIKA – status-row stílus ─────────────── -->
      <div class="card" style="margin-bottom:16px;">
        <div class="card-title" style="margin-bottom:4px;">📊 Látogatói statisztika</div>
        <div class="status-row">
          <span class="status-label">Ma</span>
          <span style="font-family:'DM Mono',monospace;font-size:20px;font-weight:700;color:#7eb8f7;"><?= number_format($visits['today']) ?></span>
        </div>
        <div class="status-row">
          <span class="status-label">Elmúlt 7 nap</span>
          <span style="font-family:'DM Mono',monospace;font-size:20px;font-weight:700;color:#a78bfa;"><?= number_format($visits['week']) ?></span>
        </div>
        <div class="status-row">
          <span class="status-label">Összes látogatás</span>
          <span style="font-family:'DM Mono',monospace;font-size:20px;font-weight:700;color:rgba(255,255,255,.6);"><?= number_format($visits['total']) ?></span>
        </div>
      </div>

      <!-- ── TEREM/TANÁR STATISZTIKÁK ─────────────────────────────── -->
      ...
      <div class="card">
        ...
        <p style="font-size:11px;color:rgba(255,255,255,.3);margin-top:6px;text-align:right;">Sárga = mai nap</p>
      </div>
      <?php endif; ?>
    ...
    <section class="section" id="section-tanarok">
      ...
      <!-- Szerkesztő kártya – id="tanar-edit-card" a scrollIntoView célpont -->
      <div class="card" id="tanar-edit-card">
        <div class="card-title">✏️ Név szerkesztése</div>
        <!-- FIX: grid layout javítva – a gomb mindig látszik -->
        <div style="display:grid;grid-template-columns:130px 1fr 90px;gap:8px;align-items:center;">
          <input type="text" id="edit-kod" class="inp inp-sm" placeholder="Kód" style="text-transform:uppercase;font-family:'DM Mono',monospace;letter-spacing:.06em;" readonly>
          <input type="text" id="edit-nev" class="inp inp-sm" placeholder="Teljes név (pl. Kovács János)">
          <button class="btn btn-gold btn-sm" onclick="saveTanarNev()" style="white-space:nowrap;width:100%;justify-content:center;">Mentés</button>
        </div>
    ...
    <section class="section" id="section-szunetek">
      ...
      <p style="font-size:13px;color:rgba(255,255,255,.4);margin-bottom:20px;">Szünet idején a tanár és osztálynézet „Szünet van" üzenetet mutat, nem az órarendet.</p>
      <?php if ($aktiv_szunet): ?>
      ...
      <div class="card">
        ...
        <table class="data-table">
          ...
          <tbody>
            <?php foreach (TICKY_SZUNETEK as $sz): ?>
            <?php
        ...
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid rgba(255,255,255,.07);">
          <p style="font-size:12px;color:rgba(255,255,255,.35);">
            💡 Szünet módosításához szerkeszd a <span style="font-family:'DM Mono',monospace;color:rgba(255,255,255,.55);">TICKY_SZUNETEK</span> tömböt a <span style="font-family:'DM Mono',monospace;color:rgba(255,255,255,.55);">utils/szunet.php</span> fájlban.
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="section" id="section-felhasznalok">
      ...
      <div class="card">
        <div class="card-title">👤 Felhasználók <span style="font-size:12px;color:rgba(255,255,255,.35);font-family:'DM Mono',monospace;font-weight:400;" id="user-count">–</span></div>
        <div class="search-wrap">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
...
function detectEpulet(szam){
  ...
  if(s.startsWith('K')){const n=s.slice(1);if(['1','2','3'].includes(n)||n==='T')return{epulet:'Kollégium',emelet:0,tag:'tag-purple',emoji:'🏠'};if(n.startsWith('1'))return{epulet:'Kollégium',emelet:1,tag:'tag-purple',emoji:'🏠'};if(n.startsWith('2'))return{epulet:'Kollégium',emelet:2,tag:'tag-purple',emoji:'🏠'};if(n.startsWith('3'))return{epulet:'Kollégium',emelet:3,tag:'tag-purple',emoji:'🏠'};return{epulet:'Kollégium',emelet:0,tag:'tag-purple',emoji:'🏠'}}
...
// ── Toast ──────────────────────────────────────────────────────────────
function toast(msg,type='ok',dur=3000){const t=document.createElement('div');t.className=`toast ${type}`;t.textContent=msg;document.body.appendChild(t);setTimeout(()=>t.remove(),dur)}

// ── Admin fetch helper (X-Ticky-Admin minden hívásban) ─────────────────
...
function renderTanarok(list){
  ...
  document.getElementById('tanar-table').innerHTML=`<table class="data-table"><thead><tr><th>Kód</th><th>Teljes név</th><th></th></tr></thead><tbody>${list.map(t=>`<tr>
    ...
    <td style="color:${t.nev?'rgba(255,255,255,.85)':'rgba(255,255,255,.25)'};">${t.nev?esc(t.nev):'– nincs megadva –'}</td>
    <td><button class="btn btn-ghost btn-sm" onclick="editTanar(${JSON.stringify(t.rovid_nev)},${JSON.stringify(t.nev||'')})" title="Szerkesztés">✏️</button></td>
  </tr>`).join('')}</tbody></table>`
}

// ── FIX: editTanar – scrollIntoView + highlight + focus ────────────────
function editTanar(kod, nev) {
  const kodEl = document.getElementById('edit-kod')
  const nevEl = document.getElementById('edit-nev')

  // Értékek beírása
  kodEl.value = kod
  nevEl.value = nev

  // A szerkesztő kártya a viewport tetejére görget – ez a legmegbízhatóbb módszer
  document.getElementById('tanar-edit-card').scrollIntoView({
    behavior: 'smooth',
    block: 'start'
  })

  // Highlight animáció a névmezőn
  nevEl.classList.remove('input-highlight')
  void nevEl.offsetWidth  // force reflow
  nevEl.classList.add('input-highlight')

  // Focus + select a névmezőre (kis delay hogy a scroll befejeződjön)
  setTimeout(() => {
    nevEl.focus()
    if (nevEl.value) nevEl.select()
  }, 350)
}
...
async function saveTanarNev(){
  ...
  if(!kod){toast('Először kattints egy ✏️ gombra a listában!','err');return}
  try{
...
function renderTermek(list){
  ...
  document.getElementById('terem-table').innerHTML=`<table class="data-table"><thead><tr><th>Terem</th><th>Épület</th><th>Emelet (DB)</th><th>Linkek</th></tr></thead><tbody>${list.map(t=>{
    const det=detectEpulet(t.terem_szam);const tagStyle=det.tag==='tag-orange'?'background:rgba(251,146,60,.15);color:#fb923c;border:1px solid rgba(251,146,60,.3);':'';
    return`<tr><td style="font-family:'Playfair Display',serif;font-size:16px;font-weight:700;">${esc(t.terem_szam)}</td><td><span class="tag ${det.tag!=='tag-orange'?det.tag:''}" style="${tagStyle}">${det.emoji} ${esc(det.epulet)}</span></td><td><input type="number" min="0" max="5" value="${t.emelet!==null&&t.emelet!==undefined?Number(t.emelet):''}" placeholder="${det.emelet!==null?det.emelet:'–'}" class="inp inp-sm" style="width:72px;" onblur="saveEmelet(${JSON.stringify(t.terem_szam)},this.value)" onkeydown="if(event.key==='Enter')this.blur()"></td><td style="display:flex;gap:6px;"><a href="/terem/${esc(t.terem_szam)}" target="_blank" class="btn btn-ghost btn-sm">🚪</a><a href="/terem/${esc(t.terem_szam)}/nap" target="_blank" class="btn btn-ghost btn-sm">📅</a></td></tr>`
  }).join('')}</tbody></table>`
}
...
async function autoDetectAll(){
  ...
  for(const t of allTermek){
    ...
    const det=detectEpulet(t.terem_szam);if(det.emelet===null)continue
    try{const res=await adminFetch(`/api/admin/terem/${encodeURIComponent(t.terem_szam)}`,{method:'PATCH',body:JSON.stringify({emelet:det.emelet})});const d=await res.json();d.ok?ok++:err++}catch(e){err++}
  }
