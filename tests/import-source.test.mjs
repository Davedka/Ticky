...
// Ékezet-érzéketlen nap-leképezés: a 'Hétfő' ÉS a 'Hétfo' (sima o) is működik.
// Korábban a 'Hétfo' írásmódú bejegyzések kimaradtak az importból (üres hétfő),
// ezért volt eltérés a beszúrt sorok számában (1059 vs 2497).
const NAP_FOLDED = { hetfo: 1, kedd: 2, szerda: 3, csutortok: 4, pentek: 5 }

export function foldDayName(value) {
  return String(value ?? '')
    .trim()
    .toLowerCase()
    .replace(/á/g, 'a')
...
export function dayToIndex(value) {
  return NAP[value] ?? NAP_FOLDED[foldDayName(value)] ?? null
}
...
export function buildExpectedLessons(entries) {
  ...
  for (const entry of entries) {
    const day = dayToIndex(entry.day)
    if (!day) continue
